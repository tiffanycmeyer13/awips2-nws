 ...
 * May 17, 2021 91567      jwu         Allow minutes in ref. time.
 *
 * </pre>
 *
 * @author pwang
 * @version 1.0
 */
public class BDeckParser extends AbstractAtcfParser {
    ...
    public AbstractAtcfRecord[] parseStream(InputStream inputStream,
        ...
        return defaultParseStream(inputStream, thisStorm, BDeckRecord.class);
    }
     ...
     */
    @Override
    protected BDeckRecord processFields(String theBulletin, int year) {
        BDeckRecord drecord = new BDeckRecord();
        ...
        /*
         * Note: B-Deck may have minutes for a storm landing record, but its
         * minute is stored in "technique num", not the warn date/time string,
         * so it needs to be set into the DTG calendar.
         */
        LocalDateTime warnTm = parseDtg(atcfField[2]);

        int dtgMinutes = 0;
        if (!atcfField[3].isEmpty()) {
            dtgMinutes = Integer.parseInt(atcfField[3]);
            drecord.setTechniqueNum(dtgMinutes);
            if (dtgMinutes > 0) {
                warnTm = warnTm.plusMinutes(dtgMinutes);
            }
        }

        drecord.setRefTime(Date.from(warnTm.toInstant(ZoneOffset.UTC)));

        if (!BEST.equals(atcfField[4])) {
            throw new IllegalArgumentException(
                    "unepxected technique in B-deck \"" + atcfField[4] + "\"");
        }
        drecord.setTechnique(BEST);
    ...
    /**
     * Update the stormName and and subRegion fields from the given records.
     *
     * @param records
     * @param thisStorm
     */
