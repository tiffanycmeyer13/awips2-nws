...
# SOFTWARE HISTORY
# 20230831 - J. Buchman
#  Set mode for temporary files to 664, to avoid conflicts with multiple users.
#  Pop up a dialog box alerting user if config_tsu.py is missing or unreadable.
# 20230317 - J. Buchman
#  Merge Ryan's changes for Hawaii time zone, screen too big for some monitor
#  configurations, "THE THE" fix.
# 20220331 - Ryan
...
import textwrap
import time
import tkinter  # need to import twice. (Here for the toop tip)
import tkinter.font
from tkinter import *
...
logFileName = os.path.join(
    logPath, "TsuGuiLog-{0}.{1}".format(getpass.getuser(), today.strftime("%Y%m%d"))
)
try:
    logging.basicConfig(
...
if os.path.isfile(ConfigFileName):
    ...
    logging.error(
        "{0} file does not exist. You need to copy the config_tsu.template.py file from the programs config/ directory to the etc/ directory, then rename to {0}, then edit and configure for your site before running this program.".format(
            ConfigFileName
        )
    )
    print(
        "{0} file does not exist! You need to copy the config_tsu.template.py file from the program's config/ directory to the etc/ directory, rename it {0}, then configure it for your site before running this program.".format(
            ConfigFileName
        )
    )
    #
    # If launched from the AWIPS menu, the user won't see the log message. Pop up an alert window
    # to notify the user. (Need to use a system utility since the app window has not been initialized yet.)
    #
    fnfPopup = "notify-send -t 5000 -u critical 'File not found' 'Please configure file {0} before using the Tsunami EAS GUI application.\\n\\nClick on this box to exit.'".format(
        ConfigFileName
    )
    ret = os.system(fnfPopup)
    sys.exit(1)

if os.access(ConfigFileName, os.R_OK) == False:
    logging.error("{0} file exists but is not readable.".format(ConfigFileName))
    fnfPopup = "notify-send -t 5000 -u critical 'File not readable' 'Configuration file {0} exists but is not readable.\\n\\nClick on this box to exit.'".format(
        ConfigFileName
    )
    ret = os.system(fnfPopup)
    sys.exit(1)
...
try:
    ...
    logging.exception(
        "Syntax error in configuration file {0}. Please edit and correct\n".format(
            ConfigFileName
        )
    )
    sys.exit(1)
...
try:
    ...
    nwrsend = (
        ""  # variable for main radio script location was not specified in config file.
    )

if nwrsend == "":
    Version += " - SEND TO RADIO DISABLED!!! UPDATE NWRSEND VARIABLE IN CONFIG FILE"
else:
    nwrsend = (
        local_dir + "/bin/nwrsend.sh"
    )  # hard code to use send script included in tsunami package instead of baseline transferNWR
...
def readArrivalList(filePath, zoneOut):
    ...
    if os.path.isfile(arrFileName):
        ...
        logging.error(
            "{0} - file does not exist! No forecast points will be included in the tsunami message.".format(
                arrFileName
            )
        )
        return []
    if os.access(arrFileName, os.R_OK) == False:
        logging.error(
            "{0} - file exists but is not readable. No forecast points will be included in the tsunami message.".format(
                arrFileName
            )
        )
        return []
    ...
    try:
        ...
        logging.error(
            "An error occurred reading from file {0}\nNo forecast points will be included in the tsunami message.".format(
                arrFileName
            )
        )
        return []
...
def daylightMe(dt):
    #
    # Several time zones, especially for islands, do not observe DST.
    #
    if tzid in ["chst", "hst", "sst", "ast"]:
        # print("Time zone {0} does not observe Daylight Savings Time.".format(tzid))
        return 0
    ...
    for d in range(0, 16):
        ...
        if b.weekday() == 6:
            ...
            if count >= target_sunday:
                target = int(
                    str(b.year) + str(b.month).zfill(2) + str(b.day).zfill(2)
                )  # hit!
                break
    if m == 3 and compare >= target:
        return 1  # Mar2022 (change > to >=)
    if m == 11 and compare < target:
        return 1
    return 0
...
#
try:
    local_list = arrival_list  # first check if it exists from config_tsu.py
except:
    local_list = []  # it does not
arrival_list = []
...
for a in local_list:
    if a.find(",") == -1:
        continue  # legacy arrival_list
    tmp = a.split(",")
    if len(tmp) < 2:
        continue  # not formatted correctly
    into = tmp[0] + " " + tmp[1]
    if tmp[2].upper() == "ADD" and into not in arrival_list:
        arrival_list.append(into)
    if tmp[2].upper() == "IGNORE" and into in arrival_list:
        arrival_list.remove(into)
...
if local_dir[-1:] != "/":
    local_dir = local_dir + "/"  # in case we forgot the ending slash
...
tmp = wcapil.split(":")
if len(tmp) > 1:
    wcapil = tmp[0] + ":" + node + tmp[1]
wcapil = wcapil.upper()

# Backwards Compatible with older Versions.
# New config items added since main release
try:
    test = local_actions  # canned local actions section
except:
    local_actions = ""
try:
    test = pending  # go to pending in NWR Browser 201504014
except:
    pending = "no"
try:
    test = advisoryPIL  # BMH PIL for advisory
except:
    advisoryPIL = "TSYWCA"
try:
    test = watchPIL  # BMH PIL for watch
except:
    watchPIL = "TSAWCA"
try:
    test = warningPIL  # BMH PIL for warning
except:
    warningPIL = "TSWWCA"
try:
    test = textsize
...
try:
    test = openPractice  # open always in practice mode
except:
    openPractice = "no"
...
def checkAwipsMode(what=""):
    ...
    if shellout == "test" or shellout == "practice":
        modeout = shellout  # otherwise default to live.
    # Toggle options
    global modeme_options
    modeme_options = ["live", "practice"]  # determines toggle options (global)
    if modeout == "test":
        modeme_options = ["test", "practice"]
    if modeout == "practice":
        modeme_options = ["practice"]
    # spit it out already
    return modeout
...
# Check whether this WFO is currently in daylight savings time.
dst = daylightMe(todaylocal)

todaylocal = todaylocal + datetime.timedelta(
    hours=dst
)  # fix for daylight savings time (1:PDT 0:PST)
tzset = tz_list[dst]
mon_long = [
    "",
    "JAN",
...
def viewHelp():
    ...
    Button(
        alertframe, text="   Close   ", command=alertframe.destroy, bg="#000", fg="#fff"
...
class ToolTip:
    ...
    def coords(self):
        ...
        x = (
            tw.winfo_pointerx() - twx // 2
        )  # Using integer division for Python 3 compatibility.
        if x < 0:
            x = 0
        elif x + twx > w:
            x = w - twx
...
def printRadio():
    ...
    tmpFile = local_dir + "data/print.txt"
    with open(tmpFile, "w") as pp:
        pp.write(pout)  # write to temporary file

    # Set the mode so that other users in the fxalpha group can write to the file later.
    try:
        os.chmod(tmpFile, 0o664)
    except:
        logging.exception(
            "(nonfatal) Could not change mode for file {0}".format(tmpFile)
        )

    pipe = subprocess.getoutput(
        "a2ps " + tmpFile + " -R -B -1 -f 20"
    )  # use commands to remove a2ps output


# Print Radio Script
def printBMH():
    pipe = subprocess.getoutput(
        "a2ps " + local_dir + "data/tsradio.txt -r -B -1"
    )  # use commands to remove a2ps output
...
def hazDefMe(a):
    ...
    if a.find("advisory") != -1:
        hazdef += " IF YOU ARE LOCATED IN THIS COASTAL AREA, MOVE OFF THE BEACH AND OUT OF HARBORS."
    return hazdef
...
def autoFillPop(pil):
    ...
    mainLabels["bt_viewStatement"].configure(
        text=" View " + wcapil_display.replace(teststatementdir, "") + " Statement"
    )
    autoFill()
...
def autoFill(combowhat=""):
    ...
    masterTone = 1
    for z in zone_ids:
        bt_zones[z].set(0)  # clear out zone buttons
    # Reset
    if combowhat == "":
        combo_need = []
        still_need = []
    ...
    info = grabTWC(
        wcapil, zone_ids
    )  # dir,dist,mag,landmark,daytime,[arrivals],warning,watch
    # print "autofill2",info
    radiotext.delete(0.0, END)  # clear out text
    # for test wording
    if len(info) == 0 or (
        len(info) == 1 and info["extra"] != ""
  ...
  version, or use one of the options from the Templates menu. code:autoFillONE"
    ...
    if info["extra"] != "":  # There was a less serious error with a notice
        if (
            info["extra"].find("appears to be a test") != -1
        ):  # special case for test message
            if combowhat != "test":  # from first loading a statement
                flagMeBasic(
                    info["extra"], "What do you want to do?", "Load with Test Wording"
                )  # prompt user for reply
                return  # stop for the moment
            else:  # user selected to load into test mode (from flagmebasic just above)
                testmode = 1
                combowhat = (
                    ""  # use combowhat as a holder to tell that user bypassed cancel
                )
        else:
            flagMeBasic(info["extra"])  # There was a less serious error with a notice
    # print info,zone_ids
    # Determine Scenario
    dowhat = ""
    if combowhat != "" and combowhat != "all":
        dowhat = combowhat  # single hazard assigned from combo option
    elif (
        len(info["warning"]) == 0
        and len(info["watch"]) == 0
        and len(info["advisory"]) == 0
    ):
        dowhat = ""
    elif advisory_handle == "no":  # Ignoring Advisories. One less thing to worry about
        if len(info["warning"]) == 0 and len(info["watch"]) == 0:
            dowhat = ""
        elif len(info["warning"]) == 0:
            dowhat = "watch"
        elif len(info["watch"]) == 0:
            dowhat = "warning"
        else:
            if combowhat == "all":
                dowhat = "warning_watch"  # user already chose combo, proceed
            else:
                comboOption(
                    "warning_watch"
                )  # must be a warning/watch combo. Ask user how to proceed
                return
    elif len(info["warning"]) == 0 and len(info["watch"]) == 0:
        dowhat = "advisory"
    elif len(info["warning"]) == 0 and len(info["advisory"]) == 0:
        dowhat = "watch"
    elif len(info["watch"]) == 0 and len(info["advisory"]) == 0:
        dowhat = "warning"
    else:  # Must be more than one!
        if len(info["warning"]) == 0:
            dowhat = "watch_advisory"
        elif len(info["watch"]) == 0:
            dowhat = "warning_advisory"
        elif len(info["advisory"]) == 0:
            dowhat = "warning_watch"
        else:
            dowhat = (
                "warning_watch_advisory"  # dreaded triple combo. No solution for that!
            )
        if combowhat != "all":
    ...
    if dowhat == "":
        ...
        if len(info["advisory"]) > 0:
            out = "While an advisory was found, no warnings or watches were detected in your area."
        elif (
            "issue" in info and len(info["issue"]) == 1 and info["issue"][0] == "final"
        ):  # 20150401 fix
            out = (
                "It appears that there was a cancellation in the most recent bulletin. "
            )
        else:
            out = "No tsunami hazards were detected in your area. "
            continuebutton = 0  # turn off option to continue
        if cancelflg == 1:
            out += "If you want to issue a cancellation message, please use one of the options from the Cancelations menu. Remember to manually delete the previous BMH Tsunami statement if still active."
    ...
    # Determine if this is a simple single warning case
        ...
        out = statementMake(
            wwa
        )  ### AAAAAA:Area Description, NNNNNN:WWA Call2Action, EEEEEE:Earthquake, LLLLLL:Arrival Times, SSSSSS:Local Actions
        # earthquake info
        if "eseg" in info and info["eseg"] != "":
            out = out.replace("EEEEEE", info["eseg"])
        else:
            out = out.replace("EEEEEE.", "").replace(
                "EEEEEE", ""
            )  # not available, skip
        # Area Desciption
        local = localphrase  # from
        if wwa in breakpoints and breakpoints[wwa] != "":
            local = "following locations, " + breakpoints[wwa]  # Mar2022
        out = out.replace("AAAAAA", "for the " + local)
        # Call to Action
        if hazdef != "":
            out = out.replace("NNNNNN", hazdef)
        else:
            out = out.replace("NNNNNN.", "XXXXXX").replace("NNNNNN", "XXXXXX")
        # Arrival Times
        arrive = ""
        tz2 = "PACIFIC STANDARD TIME"
        if tzid in tz_long:
            tz2 = tz_long[tzid] + " STANDARD TIME"  # LOCAL TIME
        if "daytime" in info and info["daytime"].find("STANDARD") == -1:
            tz2 = tz2.replace("STANDARD", "DAYLIGHT")
        # Wave Arrivals
        key = "arrivals_" + wwa
        if (
            key in info and len(info[key]) > 0
        ):  # put segment header only if there is something here
            arrive += (
                "\n\nEstimated arrival times for the first in the series of tsunami waves are as follows in "
                + tz2
            )
            for a in info[key]:
                arrive += ".\n" + a
            arrive += "."
        if arrive != "":
            out = out.replace("LLLLLL", arrive)
        else:
            out = out.replace("LLLLLL.", "").replace(
                "LLLLLL", ""
            )  # not available, skip
    ...
    # 2 Combo option - Will not use the template option for now.  (triple option not handled here)
        ...
        if "eseg" in info:
            eq = info["eseg"]
        out += (
            "The National Weather Service has issued a tsunami "
            + wwa1
            + " for the "
            + break1
            + " and a tsunami \
  "
        ...
        if tzid in tz_long:
            tz2 = tz_long[tzid] + " STANDARD TIME"  # LOCAL TIME
        if info["daytime"].find("STANDARD") == -1:
            tz2 = tz2.replace("STANDARD", "DAYLIGHT")
        arrive = ""
        track = []  # no duplicates
        ...
        # Final warning
        out += (
            "\n\nOnce again, The National Weather Service has issued a tsunami "
            + wwa1
            + " for the "
            + break1
            + " and a \
  tsunami "
    ...
    untestMe(1)
    if wcapil.find("/") != -1:
        testMe(1)  # hard coded for example statements
    elif testmode == 1 or modeme == "test":
        testMe()
...
def statementMake(wwa, directory="", what="", withwhat=""):
    ...
    out = ""
    if directory == "":
        directory = "config/template_" + wwa + ".txt"  # specified warning statement
    d = todaylocal.strftime("%B %d")
    dt = "STANDARD"
    if daylightMe(todaylocal) == 1:
        dt = "DAYLIGHT"
    tz2 = "PACIFIC " + dt + " TIME"
    if tzid in tz_long:
        tz2 = tz_long[tzid] + " " + dt + " TIME"  # LOCAL TIME
    ...
    if what != "":
        ...
        if what == "AAAAAA" or what == "XXXXXX":
            if withwhat != "":
                out = out.replace(
                    "AAAAAA", "for the following locations, " + withwhat
                )  # Mar2022
            else:
                out = out.replace(
                    "AAAAAA", "for the following locations, XXXXXX"
                )  # don't ignore if "" #Mar2022
        # Call to Action
        ...
        if what == "EEEEEE" or what == "XXXXXX":
            if what == "XXXXXX" or withwhat == "XXXXXX":
                eq = (
                    ...
                    + " AT XXXXXX "
                    + tz2
                    + " a large earthquake with a preliminary magnitude of XXXXXX occurred near XXXXXX."
                )
                out = out.replace("EEEEEE", eq)
            elif withwhat != "":
                out = out.replace(
                    "EEEEEE", withwhat
                )  # if withwhat=="" ignore this section
            else:  # Remove Section
...
def fillWWA(wwa, d=""):
    ...
    arr["advisory"] = []
    for z in zone_ids:
        if z in bt_zones and bt_zones[z].get() == 1:
            arr[wwa].append(z)
    breakpoints = breakpointMe(arr)  # advisory, warning, watch (string)
    lphrase = "XXXXXX"
    if wwa in breakpoints and breakpoints[wwa] != "":
        lphrase = breakpoints[wwa]
    ...
    if len(arr[wwa]) == 0:
        flagMeBasic(
            "Don't forget to select the Applicable Zones from the check buttons on the main window."
        )
...
def comboOption(a=""):
    ...
    destroyBoxes()
    if a != "" and a.find("_") == -1:
        a = "warning_watch"  # have to force to something
    bg = "#eee"
    alertframe = Toplevel()
    ...
    if a == "":
        lbl = "Which Combo Set would you like a template for?"
    Label(alertframe, text=lbl, bg=bg, fg="#000").grid(
        row=0, column=0, columnspan=3, padx=gp, pady=gp
    )
    if (
        a == "" and advisory_handle == "no"
    ):  # no advisory option, choice is easy (when segment option is used: default not)
        fillWarningWatch()
        return
        ...
        if combo_handle == "yes" and len(tmp) < 3:
            Button(
                alertframe, text=" All ", command=fillcomboAll, bg="#000", fg="#fff"
            ).grid(row=1, column=col, sticky=W + E)
            col += 1
        if "warning" in tmp:
            Button(
                alertframe,
                text=" Warning ",
                command=fillcomboWarning,
                bg="#000",
                fg="#fff",
            ).grid(row=1, column=col, sticky=W + E)
            col += 1
        ...
        if "watch" in tmp:
            Button(
                alertframe, text=" Watch ", command=fillcomboWatch, bg="#000", fg="#fff"
            ).grid(row=1, column=col, sticky=W + E)
            col += 1
    ...
    Button(
        alertframe, text=" Cancel  ", command=alertframe.destroy, bg="#00a", fg="#fff"
    ).grid(row=2, column=0, columnspan=3, padx=gp, pady=gp)
...
def fillWhat(d):
    ...
    for z in zone_ids:
        bt_zones[z].set(0)  # clear zone check boxes
    # Clear WWA tracking
    info["warning"] = []
    info["watch"] = []
    info["advisory"] = []
    ...
    if os.path.isfile(local_dir + d):
        ...
        for a in lines:
            if len(a) == 0 or a[0] != "#":
                out += a + "\n"
        out.replace(
            "WARNING CENTER", "WARNI CENTER"
        )  # in case warning center is in the template???
        wwafind = [0, 0, 0]
        if out.find("WARNING") != -1:
            wwafind[0] = 1
        if out.find("WATCH") != -1:
            wwafind[1] = 1
        if out.find("ADVISORY") != -1:
            wwafind[2] = 1
        if sum(wwafind) == 0:  # WWA not in the template :-\
            return flagMeBasic(
                "Something went wrong with your template selection. No WWA was found in the template. Please check your template file. code:fillWhatONE"
            )
        if sum(wwafind) > 1:
            return flagMeBasic(
                "It appears that the template file has multiple WWA type. Sorry but combo templates are not supported at this time. code:fillWhatTWO"
            )
        if wwafind[0] == 1:
            fillWWA("warning", d)
        if wwafind[1] == 1:
        ...
        if wwafind[2] == 1:
            fillWWA("advisory", d)
        if d.find("cancel") != -1:
            masterTone = 0  # turn off tone alert
        else:
            masterTone = 1
        # print masterTone,d
    return
...
#   info["advisory"]=[]
#   fillWWA("advisory")
def fillWarningWatch():
    fillCombo("warning_watch")


def fillWarningAdvisory():
    fillCombo("warning_advisory")


def fillWatchAdvisory():
    fillCombo("watch_advisory")
...
def fillCombo(a="warning_watch"):  # like fillWWA, but for unique combo situation
    ...
    for z in zone_ids:
        if z in bt_zones and bt_zones[z].get() == 1:
            arr.append(z)
    if len(arr) == 0:
        flagMeBasic("Don't forget to select the Applicable Zones.")
    # Now get on with it!
    d = todaylocal.strftime("%B %d")
    dt = "STANDARD"
    if daylightMe(todaylocal) == 1:
...
def grabTWC(pil, zone_ids):
    ...
    tz_set = ""
    if tzid in tz_long:
        tz_set = tz_long[tzid]  # what earthquake info should be set to
    tsuin = ""
    # print "here"
    ...
    try:
        if pil.find("/") != -1 or pil.find(".") != -1:
            if not os.path.isfile(pil):  # Fail!
                flagMeBasic(
                    "Specified Input File Not Found. Try changing file or PIL in the Autofill options. code:grabTWCONE"
                )
                return info
            ...
            try:
                ...
                if bulletinTm > 0:
                    ...
                    if days > 0:
                        timeAgo = "{0} day{1} and {2} hour{3}".format(
                            days, dPlu, hours, hPlu
                        )
                    elif hours > 0:
                        timeAgo = "{0} hour{1} and {2} minute{3}".format(
                            hours, hPlu, minutes, mPlu
                        )
                    else:
                        timeAgo = "{0} minute{1} and {2} second{3}".format(
                            minutes, mPlu, seconds, sPlu
                        )
                    timeGoneBy = "A {0} bulletin was received {1} ago".format(
                        pil, timeAgo
                    )
                    # print(timeGoneBy)
        ...
        for s in segs:
            ...
            for line in ss:
                if (
                    ...
                    if (
                        ...
                        break
            if vline == "":
                continue  # no vtec detected in segment
            wwa = ""
            # if s.find("TSUNAMI WARNING")!=-1: wwa="warning"
            # if s.find("TSUNAMI WATCH")!=-1: wwa="watch"
            if vline.find("TS.A") != -1:
                wwa = "watch"
            elif vline.find("TS.W") != -1:
                wwa = "warning"  # yeah VTEC!
            elif vline.find("TS.Y") != -1:
                wwa = "advisory"
            if wwa == "":
                continue  # might be a blank line. Either way, not whay i am expecting
            if vline.find("/T.") != -1:
                testvtec = "yes"
            # Determine if new issuance or continued
            issue = "new"
            # print vline
            if vline.find(".EXP.") != -1 or vline.find(".CAN.") != -1:
                issue = "final"
            if (
                vline.find(".CON.") != -1
                or vline.find(".UPG.") != -1
            ...
            if vline.find(".NEW.") != -1:
                issue = "new"
            if issue == "final":
                continue  # should not count these zones...they are no longer in effect
            ...
            if len(tmp) > 0:  # hit, zones present and detected!
                ...
                for a in arr:
                    ...
                    if len(a) > 3:
                        if (
                            a.find("Z") != -1
                        ):  # should have Z in zone code. Skips date code.
                            pre = a[0:3]
                            if (
                                a not in info[wwa] and a in zone_ids
                            ):  # hit, these are the zones we want
                                info[wwa].append(a)
                                if issue not in info["issue"]:
                                    info["issue"].append(issue)
                    else:
                        id = pre + str(a)
            ...
            for j in range(0, len(ss)):  # go through each section
                ...
                if isHawaii or isCarib:
                    ...
                    if isHawaii:
                        ...
                        #
                        if (
                            ss[j].find("ESTIMATED TIME OF ARRIVAL OF THE INITIAL WAVE")
                            >= 0
                            or ss[j].find(
                                "IF TSUNAMI WAVES IMPACT HAWAII THE ESTIMATED EARLIEST ARRIVAL"
                            )
                            >= 0
                        ) and ((j + 1) < len(ss)):
                            ...
                            if len(fields) >= 6 and len(fields[0]) == 4:
                                ...
                                try:
                                    ...
                                    arrMonth = fields[5]
                                arriveLine = (
                                    "HAWAIIAN ISLANDS ON {0} {1} AT {2} {3} {4}".format(
                                        arrMonth, fields[4], arrTime, fields[1], arrZone
                                    )
                                )
                ...
                #
                    ...
                    isGuam and ss[j].find("-ETA- OF THE INITIAL TSUNAMI WAVE") >= 0
                ):  # isolate arrival segment
                    # print("Found arrival segment: " + ss[j])
                    for ii in range(
                        1, 3
                    ):  # find first station...in case there are many empty lines
                        k = j + ii  # ss[k] should include entire arrival times segment
                        if (
                            len(ss) > k
                            and ss[k].replace(" ", "").replace("\n", "") != ""
                        ):  # non empty line, HIT!
                            ...
                            for line in sss:  # go through each line
                                if (
                                    isGuam
                                ):  # Skip the two header lines in the Guam bulletin.
                                    if (
                                        line.find("LOCATION") >= 0
                                        and line.find("REGION") >= 0
                                    ):
                                        continue
                                    if line.find("------------") >= 0:
                                        continue
                                ...
                                #
                                # The following code creates a list of the fields in the ETA line, then
                                # trims (pops) the trailing five fields. This leaves the location name
                                # and the state/territory, which are then joined into the station ID.
                                ...
                                if hitme == 1:  # This station is on the Arrival list.
                                    ...
                                    if isGuam:
                                        ...
                                        line = re.compile("[\W]").sub(
                                            " ", line
                                        )  # band-aid for occasional bad characters from NTWC
                                        pat2 = "  ([0-9]+)\s+([A,P,M]+)\s+([A-Z][A-Z]?[S,D]T)\s+([A-Z]+)\s+([0-9]+)"  # find time string
                                        tmp = re.compile(pat2).findall(
                                            line
                                        )  # [('724', 'AM', 'PST', 'MARCH', '11')]
        ...
        for i in range(0, len(segs)):  # isolate earthquake segment
            ...
            #
            if (
                segs[i].find("EARTHQUAKE") != -1 and segs[i].find("MAGNITUDE") != -1
            ):  # main segments
                ss = (
                    segs[i]
                    .replace("\r", "")
                ...
                for j in range(0, len(ss)):  # isolated paragraph in segment
                    ...
                    if isHawaii:
                        if (
                            ss[j].find("AN EARTHQUAKE HAS OCCURRED WITH THESE") != -1
                            and len(ss) > j
                        ):
                        ...
                        ss[j].find("EARTHQUAKE") != -1 and ss[j].find("MAGNITUDE") != -1
                    ):  # with some phrasing cleanup
                        eseg = (
                            ss[j]
                            .replace("WITH PRELIMINARY", "WITH A PRELIMINARY")
                            .replace("MAGNITUDE", "MAGNITUDE OF")
                            .replace("OF OF", "OF")
                        )
                        if eseg[0:2] == "* ":
                            eseg = eseg[2:]
        ...
        if tzset != "" and eseg.find(tzset) == -1:  # need to convert to local time
            tzpatarr = []
            for a, b in tz_offlist.items():
                tzpatarr.append(a)  # grab available timezones from arr at top
            tzpat = (
                str(tzpatarr)
                .replace("[", "")
                .replace("]", "")
            ...
            if len(tmp) > 0 and len(tmp[0]) > 2:  # Enough for a match
                ...
                ut = time.mktime(
                    time.strptime(dstring, "%B %d %Y %I%M %p")
                )  # unix time of earthquake
                tzname = (
                    str(tmp[0][1]) + " " + str(tmp[0][2]) + " " + "TIME"
                )  # time zone name
                if tmp[0][1] in tz_offlist:
                    ...
                    if tzid in tz_long:
                        ...
                        if dst == 1:
                            tzname = tzname.replace("STANDARD", "DAYLIGHT")
                tzdt = datetime.datetime.fromtimestamp(
                    ut
                )  # new time in daytime format...almost there
                d = str(int(tzdt.strftime("%d")))  # annoying preceding "0" fix
                t2 = str(int(tzdt.strftime("%I")))
                # Save for later
                daytime = tzdt.strftime("%B DDD %Y AT TTT%M %p XXX")
                daytime = (
                    daytime.replace("DDD", d).replace("TTT", t2).replace("XXX", tzname)
                )
                info[
                    "daytime"
                ] = daytime  # April 16 2012 AT 939 AM PACIFIC DAYLIGHT TIME
                dtarr = daytime.split(" ")  # element by element
                # now fix up eseg
                # print dd,dtarr[0]+" "+dtarr[1]
                eseg = eseg.replace(
                    tmp[0][0], dtarr[4] + " " + dtarr[5]
                )  # time replaced
                eseg = eseg.replace(
                    str(tmp[0][1]) + " " + str(tmp[0][2]), dtarr[6] + " " + dtarr[7]
                )  # Timezone replaced
    ...
    if (
        ...
        info[
            "extra"
        ] = "No tsunami WWA is in effect for your area, or the parsing failed.\nClick the View TSU statement to check or load a previous version"
        return info
        pass
    if tsuin != "" and (
        (testvtec == "yes" and tsuin.find("TEST") != -1) or (tsuin.find("TEST") != -1)
    ):  # test product
        info[
            "extra"
        ] = "The latest message from the Tsunami Warning Center appears to be a test.\nYou may Load it \
...
def formatGuamArrival(stnid, line):
    ...
    try:
        ...
        logging.exception(
            "An error occurred formatting arrival time for station {0}".format(stnid)
        )
        return ""
...
def parseHawaiiEQ(segment):
    ...
    for line in lines:
        if line.find("ORIGIN TIME") >= 0:
            ...
            if field[5] != "HST":
                ozone = field[3]
            otime = "{0} {1} {2} ON {3} {4} {5}".format(
                field[3], field[4], ozone, field[6], field[7], field[8]
            )
    ...
    eqStatement = (
        "AN EARTHQUAKE WITH A PRELIMINARY MAGNITUDE OF {0} OCCURRED {1} AT {2}".format(
            magnitude, location, otime
        )
    )
...
def changeZone():
    ...
    try:
        if len(a) < 1 or active == "":
            return  # no hazard selected yet
        ...
        text = text.strip()
        findme = (
            "TSUNAMI " + active.upper() + " FOR THE FOLLOWING LOCATIONS,"
        )  # Mar2022
        a = text.split(
            findme
        )  # Isolate findme phrase. Looking for one hit to use to replace phrase
        if len(a) > 1:  # a[0]before phrase, b[0]phrase
            b = a[1].split(
                "."
            )  # 20180919 fix.  There was an issue with just using "." in 20150312.
            replaceme = "FOR THE FOLLOWING LOCATIONS," + b[0]  # Mar2022
            replacewith = (
                "FOR THE FOLLOWING LOCATIONS," + " " + (replacewith.strip())
            )  # Mar2022
...
def zonelistFull(zones):
    ...
    try:
        ...
        if "XXXXXX" in pre or len(pre) == 0:
            ...
            if len(pre) == 2:
                preme = preme.replace(", AND", " AND")
            out = preme
        return out
    except Exception:
        flagMeBasic("Zone names did not list properly. Code:zonelistFull")
    return ""
...
def breakpointMe(info, wwa=""):
    ...
    for wwa in ["watch", "warning", "advisory"]:
        if (
            len(info[wwa]) != 0
            and len(info[others[wwa][0]]) == 0
            and len(info[others[wwa][1]]) == 0
        ):  # simple only one hazard
            if len(info[wwa]) == len(zone_ids):  # simple! all zones plan A
                if localphrase == "":
                    out[wwa] = "the local coastal areas"
                else:
                    out[wwa] = localphrase
                ...
                for b, arr in list(bpoints.items()):  # find which breakpoint matches
                    if 0 in arr and info[wwa] == arr[0] and b in compass:
                        out[wwa] = "coastal areas " + compass[b][0] + " of " + b
                    if 1 in arr and info[wwa] == arr[1] and b in compass:
                        out[wwa] = "coastal areas " + compass[b][1] + " of " + b
...
def untestMe(passit=0):  # Remove Test Wording to Statement
    ...
    if text.find("TEST") == -1 and passit == 0:
        if passit == 0:
            flagMeBasic("No Test wording found in statement")
        return
...
def highlightMe(
    what, s, fg="#000", bg="#fff"
):  # visually hightlight text in statement window
...
def modemeLabelChange():  # change settings when going to live/practice/test mode #NOSONAR S3776 can be waived 20>15 is acceptable
    global modeme, mainLabels, bgmain, fontsize
    mainLabels["modeme"].configure(text=modeme.title() + " Mode")
    fg = "#000"
    if modeme == "practice":
        bgmain = bgmain_list["practice"]
        mainLabels["modeme"].configure(
            fg="#fff", font=("Arial", str(int(fontsize) + 10), "bold")
        )
        if "bt_eas" in mainLabels:
            mainLabels["bt_eas"].configure(text=" Pretend Send to Radio ")
    elif modeme == "test":
        bgmain = bgmain_list["test"]
        mainLabels["modeme"].configure(
            fg="#c00", font=("Arial", str(int(fontsize) + 10), "bold")
        )
        if "bt_eas" in mainLabels:
            mainLabels["bt_eas"].configure(text=" Send to Radio as Test")
        fg = "#aaa"
    else:
        bgmain = bgmain_list["live"]
        mainLabels["modeme"].configure(
            fg="#c33", font=("Arial", str(int(fontsize) + 10), "bold")
        )
        if "bt_eas" in mainLabels:
            mainLabels["bt_eas"].configure(text=" Send to Radio ")
    # change colors
    ...
    for a, b in list(mainLabels.items()):
        try:
            ...
            if a[0:3] != "bt_":
                ...
                if a != "title":
                    mainLabels[a].configure(fg=fg)
            else:
                mainLabels[a].configure(
...
def toggleMode():
    ...
    # Hard Rules
    if modemeAwips == "practice":
        modeme = "practice"
    # Toggle
    if modeme == "live" or modeme == "test":
        modeme = "practice"
...
def autofillSettings():
    ...
    if wcapil2.find(":") != -1:  # older version
        ...
        if len(tmp) > 1:
            ...
            if version == "0":
                version = "Latest"
            if version.find("-") == -1 and version != "Latest":
                version = "-" + version
    alertframe = Toplevel()
    ...
    alertframe.option_add("*Font", ("Arial", 12))
    alertframe.geometry(windowXgeo())
    Label(alertframe, text="Product PIL for Autofill to parse", bg=bg, fg="#00a").grid(
    ...
    Label(alertframe, text="PIL Version To Use", bg=bg, fg="#00a").grid(
        row=1, column=0, sticky=E
    )
    optset["version"] = StringVar()
    OptionMenu(
        alertframe, optset["version"], *["Latest", "-1", "-2", "-3", "-4", "-5"]
    ).grid(row=1, column=1, sticky=W)
    optset["version"].set(version)
    Button(
        alertframe,
        text="  Save  ",
        command=autofillChangesettings,
        bg="#37c",
        fg="#fff",
    ).grid(row=2, column=0, sticky=E)
    Button(
        alertframe, text="  Cancel  ", command=alertframe.destroy, bg="#000", fg="#fff"
...
def loadExampleGo():
    ...
    if 1:
        ...
        if "wcapil" in optset and optset["wcapil"] != "":
            ...
            if (
                wcapil.find("/") != -1 or wcapil.find(".") != -1
        ...
        destroyBoxes()
        if modeme == "live":
            toggleMode()  # force to practice mode
...
def loadExampleStatement():
    ...
    alertframe.option_add("*Font", ("Arial", 12))
    alertframe.geometry(windowXgeo())
    Label(alertframe, text="Select Example Statement", bg=bg, fg="#00a").grid(
        row=0, column=0, columnspan=2, padx=gp
    )
    noteme = Label(alertframe, text="Note", bg=bg, fg="#00a", font=("Arial", "8"))
    noteme.grid(row=0, column=2, sticky=E, padx=gp)
    ToolTip(
        noteme,
        text="The statements in the list below are pulled from txt files found in the examples directory.",
    )
    ...
    OptionMenu(alertframe, optset["wcapil"], *arr).grid(
        row=1, column=0, columnspan=3, pady=gp
    )
    if len(arr) > 0:
        optset["wcapil"].set(arr[0])
    Button(
        alertframe, text="  Load  ", command=loadExampleGo, bg="#37c", fg="#fff"
...
def previewBMH():
    ...
    Label(
        alertframe1,
        text="Script NOT sent to NWR/EAS!\nYou are only previewing...",
        bg="#cdf",
        fg="#000",
    ...
    Button(
        bframe, text="   Close   ", command=alertframe1.destroy, bg="#000", fg="#fff"
    ).grid(row=0, column=0)
    Button(bframe, text="   Print   ", command=printBMH, bg="#afa", fg="#000").grid(
        row=0, column=1
    )
...
def loadTSU(wcapil2):
    ...
    if wcapil2.find("/") != -1 or wcapil2.find(".") != -1:
        if not os.path.isfile(wcapil2):  # Fail!
            flagMeBasic(
                "Specified Input File Not. Try changing file or PIL in the Autofill options."
            )
            return info
...
def viewTSU(tsuin, wcapil2):
    ...
    Label(
        alertframe,
        text="View " + wcapil2.replace(teststatementdir, "") + " Statement",
        fg="#000",
        ...
        if wcapil2.find("/") == -1 and wcapil2.find(".") == -1:  # NOT loaded statement
            ...
            if wcapil2.find(":") != -1:  # find version number
                ...
                Button(
                    bsmallframe,
                    text="Earlier",
                    command=lambda i=earlier: loadTSU(i),
                    fg="#000",
                    relief=FLAT,
                    font=("Arial", "10"),
                ).grid(row=0, column=0)
                if version > 0:
                    ...
                    Button(
                        bsmallframe,
                        text="Later",
                        command=lambda i=later: loadTSU(i),
                        fg="#000",
                        relief=FLAT,
                        font=("Arial", "10"),
                    ).grid(row=0, column=1)
    ...
    Button(
        bframe,
        text=" Load Statement To Main Window",
        command=lambda i=wcapil2: autoFillPop(i),
        bg="#000",
        fg="#fff",
    ).grid(row=0, column=0)
    Button(
        bframe, text="   Close   ", command=alertframe.destroy, bg="#000", fg="#fff"
    ).grid(row=0, column=1)
    text = Text(
        alertframe,
        height=45,
...
def spellMe():
    ...
    try:
        ...
        for w in words:
            pipe = subprocess.getoutput(
                "echo " + w + " | ispell -a"
            )  # Pipe to built-in spell checker
            tmp = pipe.split(":")
            if str(pipe).find("command not found") != -1:  # ispell is not istalled :(
                flagMeBasic("Sorry, but iSpell is not installed on this machine :(")
                return
            if len(tmp) > 1:
                spell[w] = tmp[1].strip().split(",")  # hit
        # Any hits?
        if len(spell) == 0:
            flagMeBasic("No Spelling Errors Found", "Congratulations!")
        spellAction()
    except:
        flagMeBasic("Spell Checker Failed. Code:spellMe")
...
def spellBox(word, suggestions):
    ...
    alertframe.option_add("*Font", ("Arial", 12))
    alertframe.geometry(windowXgeo())
    Label(alertframe, text=word, bg="#ff5").grid(
        row=0, column=0, columnspan=3, padx=gp, pady=gp
    )
    ...
    Button(
        alertframe,
        text="Replace All",
        command=lambda i=word: spellReplace(i),
        bg="#37c",
        fg="#fff",
    ).grid(row=2, column=0, padx=gp, pady=gp)
    Button(
        alertframe, text="Ignore All", command=spellAction, bg="#000", fg="#fff"
    ).grid(row=2, column=1, padx=gp, pady=gp)
...
def flagMeSend(mesg):  # window to display errors
    ...
    Button(
        ...
        command=alertframe.destroy,
        bg="#ddd",
        fg="#000",
    ).grid(row=1, column=0, padx=gp, pady=gp)
    Button(
        alertframe,
        text="Continue Anyway",
        command=almostsendtoRadio,
        bg="#ddd",
        fg="#000",
    ).grid(row=1, column=1, padx=gp, pady=gp)
    logging.error(mesg)
...
def flagMeBasic(msg, title="Attention...", prompt=0):  # window to display errors
    ...
    if prompt == 0:
        Button(
            ...
            bg="#ccc",
            fg="#000",
        ).grid(row=1, column=0, columnspan=3, padx=gp, pady=gp)
    elif str(prompt).isdigit():
        Button(
            alertframe, text="Cancel", command=alertframe.destroy, bg="#ccc", fg="#000"
        ).grid(row=1, column=0, padx=gp, pady=gp)
        Button(
            alertframe,
            text="Continue Anyway",
            command=lambda i="test": autoFill(i),
            bg="#37c",
            fg="#fff",
        ).grid(row=1, column=1, padx=gp, pady=gp)
    else:
...
def sendtoRadioCheck():
    ...
    try:
        ...
        if (
            text.find("none are detected") != -1
        ):  # check is a warning or watch is in the text box
            err += "No hazard was found in the main text window. "
            if advisory_handle == "no":
                err += "Only warnings and watches are EAS'd\n\n"
        ...
        for c, zlist in list(county_list.items()):
            for z, a in list(bt_zones.items()):
                if a.get() == 1 and z in zlist and c not in arr:
                    arr.append(c)
        if len(arr) <= 0:
            err += "No zones selected or zone/counties associatations are not configured correctly\n\n"
    except Exception:
        err += "Radio Check Failed. Code:sendtoRadioCheck\n\n"
    if err != "":
        flagMeSend(err)
        return
...
def almostsendtoRadio():
    ...
    if modeme == "live":  # Send to radio with tones
        if active.find("warning") != -1 or active.find("watch") != -1:
            Label(
                alertframe,
                text="YOU ARE IN LIVE MODE!!!",
                bg=bg,
                fg="#000",
                font=("Arial", str(int(fontsize) + 6)),
                justify=CENTER,
            ).grid(row=0, column=0, padx=gp, pady=gp)
            mainLabels["no_send"] = Button(
                alertframe,
                text=" Go Ahead...Send To Radio! ",
                command=countdowntoRadio,
                bg="#c00",
                fg="#ff9",
            )
        ...
        Label(
            alertframe,
            text="YOU ARE IN PRACTICE MODE.\n NOTHING will get sent to the Radio.",
            bg=bg,
            fg="#000",
        ).grid(row=0, column=0, padx=gp, pady=gp)
        mainLabels["no_send"] = Button(
            alertframe,
            text="  PRETEND to Send to Radio!  ",
            command=countdowntoRadio,
            bg="#c00",
            fg="#fff",
        )
    ...
    Button(
        ...
        command=abortCountdown,
        bg="#ddd",
        fg="#000",
    ).grid(row=4, column=0, padx=gp, pady=gp)
    mainLabels["no_timerLabel"] = Label(
        alertframe, text="", bg=bg, fg="#700", font=("Arial", str(int(16)))
    )
...
def restoreMe():
    ...
    try:
        ...
        wwarr = []
        if out.find("WARNING") != -1:
            wwarr.append("warning")
        if out.find("WATCH") != -1:
            wwarr.append("watch")
        if out.find("ADVISORY") != -1:
            wwarr.append("advisory")
        active = ""
        for wwa in wwarr:
            if len(wwarr) > 1:
                combo_need
        ...
        flagMeBasic("Restore function failed. Code:restoreMe")
...
def saveMe():
    ...
    try:
        ...
        text = radiotext.get(0.0, END)
        if len(text) < 30:
            return  # make sure something worthwhile is there to save
        for z, a in list(bt_zones.items()):
            if a.get() == 1:
                out += z + ","
        out += "|||"
...
def clearTrack():
    ...
    try:
        ...
        alertframe.option_add("*Font", ("Arial", 12))
        alertframe.geometry(windowXgeo())
        Label(alertframe, text="  Tracking Cleared   ", fg="#000").grid(
            row=0, column=0, padx=gp, pady=gp
        )
        Button(
            alertframe,
            text="   OK   ",
        ...
        flagMeBasic("Tracking failed to clear. Code:clearTrack")
...
def openNWRBrowser():
    ...
    if os.path.exists("/awips/adapt/NWRWAVES/browser/AWIPS2-browser.tcl"):
        cmd2 = "/awips/adapt/NWRWAVES/browser/AWIPS2-browser.tcl"
    elif os.path.exists("/awips/adapt/NWRWAVES/browser/AWIPS1-browser.tcl"):
        cmd2 = "/awips/adapt/NWRWAVES/browser/AWIPS1-browser.tcl"
    elif os.path.exists("/awips/adapt/NWRWAVES/browser/AWIPS-browser.tcl"):
        cmd2 = "/awips/adapt/NWRWAVES/browser/AWIPS-browser.tcl"
    try:
        if cmd2 != "":
            os.system(cmd2)
        else:
            logging.error("NWR Browser Not Found")
    except Exception:
        logging.exception("NWR Browser did not open")
...
def zoneMe():
    ...
    try:
        ...
        flagMeBasic("Zone name listing failed. Code:zoneMe")
    return ""
...
def countyMe():
    ...
    try:
        ...
        for c, zlist in list(county_list.items()):
            for z, a in list(bt_zones.items()):
                if a.get() == 1 and z in zlist and c not in counties:
                    counties.append(c)
        counties = (
            str(counties)
            .replace("[", "")
        ...
        flagMeBasic("County name listing failed. Code:countyMe")
    return ""


def highestPri(active, text):
    wwa = ""
    if active.find("warning") != -1:
        wwa = "warning"
    elif active.find("watch") != -1:
        wwa = "watch"
    elif active.find("advisory") != -1:
        wwa = "advisory"
    else:  # plan B
        if text.find("REPEAT, TSUNAMI WARNING") != -1:
            wwa = "warning"
        elif text.find("REPEAT, TSUNAMI WATCH") != -1:
            wwa = "watch"
        elif text.find("REPEAT, TSUNAMI ADVISORY") != -1:
            wwa = "advisory"
        elif text.find("TSUNAMI WARNING") != -1:
            wwa = "warning"  # plan c
        elif text.find("TSUNAMI WATCH") != -1:
            wwa = "watch"
        elif text.find("TSUNAMI ADVISORY") != -1:
            wwa = "advisory"
        else:
            wwa = "warning"  # gotta do something, assume the worst
    return wwa
...
def crsCodeme(active, text): #NOSONAR Waive S3776 - complexity of 16>15 can can be reasonably waived at this time
    global optset
    todayutc = (
        datetime.datetime.utcnow()
    )  # UTC time (awips already default to UTC). need to assign it now because GUI may be opened for a long time
    out = "\x1baT_ENG"
    ...
    try:
        ...
        if wwa == "advisory":
            if len(advisoryPIL) < 7:
                out += node + advisoryPIL
            else:
                out += advisoryPIL
        elif wwa == "watch":
            if len(watchPIL) < 7:
        ...
        if wwa == "advisory" or masterTone == 0:
            out += "        CD IN"  # no tone alert
        else:
            out += "        CD IA"  # periodicity, active, delete, msg confirm, interupt, tonealert.
        # county string
        out += countyMe()
        # Expiration
        out += "c"
        ...
        if "crsexpire" in optset:
            ...
            forwardtime = todayutc + datetime.timedelta(minutes=15)
        ...
        for a in tmp:  # [('939', 'AM'), ('808', 'AM'), ('723', 'AM')]
            if (
                len(a) > 1 and len(a[0]) > 2
            ):  # if already has : re will only find minutes
                replA = a[0] + " " + a[1]  # what to replace
                if len(a[0]) > 3:
                    replB = a[0][0:2] + ":" + a[0][2:] + " " + a[1]  # four digit time
                else:
                    replB = a[0][0:1] + ":" + a[0][1:] + " " + a[1]  # four digit time
        ...
        flagMeBasic("BMH formatting failed. Code:crsCodeMe")
    return ""
...
def abortCountdown():
    ...
    try:
        root.after_cancel(afterme)  # cancel loop, not present if transmit not started
    except Exception:
        a = 1
...
def countdowntoRadio():
    ...
    mainLabels["no_send"].configure(
        state=DISABLED
    )  # turn off send button while timer is going
    mainLabels["no_timerLabel"].configure(
        text="Transmit in " + str(counter)
    )  # Visual Timer
    counter -= 1
    if counter >= 0:
        timerMe()  # bounce back to timerMe
    else:
        sendtoRadio()  # Go ahead with it
...
def sendtoRadio():
    ...
    try:
        ...
        if (
            modeme == "live"
        ):  # !!!!!!!!!!!!!!!!! THIS IS THE BIG COMMAND THAT SENDS TO THE RADIO!!!!!!!!!!!!!!!!!!!!!!!
            a = 1
            if pending == "no":
                cmd = (
                    nwrsend + " -a " + local_dir + "data/tsradio.txt"
                )  # -a automatically transfer, -d pending directory
            else:
                cmd = (
                    nwrsend + " -d " + local_dir + "data/tsradio.txt"
                )  # -a automatically transfer, -d pending directory
        ret = 0
        ...
        flagMeBasic(
            "Send to radio function failed. Statement likely did not make it to the radio. Code:sendtoRadioONE"
        )
    try:
        # Some book keeping
        ...
        for a in wwarr:
            if a not in combo_issued:
                combo_issued.append(a)
        for a in combo_need:
            if a not in combo_issued:
                still_need.append(a)
        ...
        for wwa in ["warning", "watch", "advisory"]:
            if (
                wwa in track
                and len(track[wwa]) > 1
                and int(track[wwa][0]) > int(time.time())
            ):  # passes time check
                arr = track[wwa][1].split("-")
                for a in arr:
                    if zones.find(a) != -1 and recentflg.find(wwa) == -1:
                        recentflg += " " + wwa  # passes county check, alert!
        ...
        if modeme == "live":
            ...
            for wwa, arr in list(track.items()):
                if len(arr) > 1:
                    out += wwa + "|" + str(arr[0]) + "|" + arr[1] + "\n"
            tmpFile = local_dir + "data/last_issued.txt"
            with open(tmpFile, "w") as pp:  # create and write to temporary file
                pp.write(out)
        ...
        if len(still_need) > 0:
            if recentflg == "":
                alertframe1.geometry(windowXgeo())  # we're going to need a bigger box
            else:
                alertframe1.geometry(windowXgeo())  # we're going to need a bigger box
        elif recentflg != "":
            alertframe1.geometry(windowXgeo())
        ...
        if ret / 256.0 == 0:  # all went well
            ...
            if modeme == "live":
                if pending == "no":
                    Label(
                        alertframe1,
                        text="Statement Successfully Sent to BMH",
                        bg=bg,
                        fg="#000",
                    ).grid(row=0, column=0, columnspan=colen, padx=gp, pady=gp)
                else:
                    Label(
                        alertframe1,
                        text="Statement Successfully Sent to Pending. Please open the NWR Browser to complete.",
                        bg=bg,
                        fg="#000",
                    ).grid(row=0, column=0, columnspan=colen, padx=gp, pady=gp)
            else:
            ...
            if len(still_need) > 0:
                Label(
                    alertframe1,
                    text="You may still need to issue a Tsunami:",
                    bg=bg,
                    fg="#000",
                ).grid(row=1, column=0, columnspan=colen, padx=gp, pady=gp)
                col = 0
                for a in still_need:
                    if a == "watch":
                        Button(
            ...
            if pending == "yes":
                Button(
                    alertframe1,
                    text=" Open NWR Browser ",
                    command=openNWRBrowser,
                    bg="#000",
                    fg="#fff",
                ).grid(row=3, column=0, columnspan=colen, padx=gp, pady=gp)
            Button(
                alertframe1,
                text="   Close   ",
                command=alertframe1.destroy,
                bg="#000",
                fg="#fff",
            ).grid(row=4, column=0, columnspan=colen, padx=gp, pady=gp)
            if recentflg != "":
                Label(
                    alertframe1,
    ...
    still be playing on the radio.\nYou may want to manually remove it from BMH.",
                    bg=bg,
                    fg="#a00",
                ).grid(row=5, column=0, columnspan=colen, padx=gp, pady=gp)
            # log it
            pil = wcapil
            tmp = pil.split(":")
            if len(tmp) > 1:
            ...
            logit = (
                pil
                + " in at: "
                + datetime.datetime.utcfromtimestamp(statementin).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            )
            logit += "\nNWR activated at: " + datetime.datetime.utcfromtimestamp(
                now
            ...
            alertframe1.configure(bg=bg)
            msg = (
                "Practice is complete, but something went strangely.\nCode:nwrTransfer"
            )
            if modeme == "live":
                if nwrsend == "":
                    ...
                    msg = (
                        "There was an error. "
                        + active.capitalize().replace("_", "+")
                        + " Product not sent to BMH.\nCode:nwrTransfer "
                    )
            Label(alertframe1, text=msg + str(ret), bg=bg, fg="#000").grid(
                row=0, column=0, padx=gp, pady=gp
            )  # error 32512, check nwrsend command
            Button(
                alertframe1,
                text="   OK   ",
                command=alertframe1.destroy,
        ...
        flagMeBasic(
            "There was an error. Statement likely sent to radio (please verify), but an error occurred after. Code:sendtoRadioTWO"
        )
...
if modeme in bgmain_list:
    bgmain = bgmain_list[modeme]
else:
    bgmain = bgmain_list["live"]
...
global printout, alertframe, radiotext, mainLabels, mainButtons, cancelflg, masterTone
...
# File Menu
mainLabels["menufile"] = Menubutton(
    mainLabels["mframe"], text="File", justify=RIGHT, bg=bgmain
)
...
# Options Menu
mainLabels["menuoptions"] = Menubutton(
    mainLabels["mframe"], text="Options", justify=RIGHT, bg=bgmain
)
mainLabels["menuoptions"].menu = Menu(mainLabels["menuoptions"])
mainLabels["menuoptions"].menu.add_command(
    label="Load Latest From NTWC Product", command=loadLatestStatement
)
mainLabels["menuoptions"].menu.add_command(
    label="Load Example Statement", command=loadExampleStatement
)
mainLabels["menuoptions"].menu.add_command(
    label="Preview BMH Script", command=previewBMH
)
mainLabels["menuoptions"].menu.add_command(
    label="Autofill Settings", command=autofillSettings
)
mainLabels["menuoptions"].menu.add_command(label="Add Test Wording", command=testMe)
mainLabels["menuoptions"].menu.add_command(
    label="Remove Test Wording", command=untestMe
)
mainLabels["menuoptions"].menu.add_command(
    label="Clear Tracked WWA", command=clearTrack
)
mainLabels["menuoptions"]["menu"] = mainLabels["menuoptions"].menu
mainLabels["menuoptions"].grid(row=0, column=2, sticky=NW, padx=3)
# Templates
mainLabels["menutemplate"] = Menubutton(
    mainLabels["mframe"], text="Backup Templates", justify=RIGHT, bg=bgmain
)
mainLabels["menutemplate"].menu = Menu(mainLabels["menutemplate"])
mainLabels["menuimminent"] = Menubutton(
    mainLabels["mframe"], text="Imminent Tsunami", justify=RIGHT, bg=bgmain
)
mainLabels["menuimminent"].menu = Menu(mainLabels["menuimminent"])
mainLabels["menucancel"] = Menubutton(
    mainLabels["mframe"], text="Cancellations", justify=RIGHT, bg=bgmain
)
...
for a in arr2:
    ...
    if a[0:9] == "template_" and a.find("~") == -1 and a.find(".swp") == -1:
        if b == "warning" or b == "watch" or b == "advisory":
            arr.append("AAAetc/" + a)  # trick for better sorting
        else:
            arr.append("etc/" + a)
...
for a in arr2:
    ...
    if (
        ...
        and a.find("~") == -1
        and a.find(".swp") == -1
        and "etc/" + a not in arr
        and "AAAetc/" + a not in arr
    ):
        if b == "warning" or b == "watch" or b == "advisory":
            arr.append("AAAconfig/" + a)
...
for a in arr:
    ...
    if len(tmp) > 1:
        if tmp.find("cancel") != -1:
            mainLabels["menucancel"].menu.add_command(
                label=tmp[0].upper()
                + tmp[1:].replace(".txt", "").replace("_", " ")
                + " template",
                command=lambda i=a: fillWhat(i),
            )
            cancelflg = 1
        elif tmp.find("imminent") != -1:
            mainLabels["menuimminent"].menu.add_command(
                label=tmp[0].upper()
                + tmp[1:].replace(".txt", "").replace("_", " ")
                + " template",
                command=lambda i=a: fillWhat(i),
            ...
            mainLabels["menutemplate"].menu.add_command(
                label=tmp[0].upper() + tmp[1:].replace(".txt", "").replace("_", " "),
                command=lambda i=a: fillWhat(i),
            )
...
mainLabels["modeme"] = Button(
    ...
    bd=0,
    command=toggleMode,
    font=("Arial", str(int(fontsize) + 10), "bold"),
)
mainLabels["modeme"].grid(row=rows, column=0, columnspan=absmaxcol, sticky=E)

rows += 1
# Title
mainLabels["title"] = Label(
    root,
    text=" Tsunami EAS GUI ",
    fg="#07a",
    bg=bgmain,
    font=("Arial", str(int(fontsize) + 16)),
)
mainLabels["title"].grid(row=rows, column=0, columnspan=absmaxcol, sticky=NW)
mainLabels["bt_viewStatement"] = Button(
    root,
    text=" View " + wcapil.replace(teststatementdir, "") + " Statement",
    command=showTSU,
    bg="#222",
    fg="#ff0",
)
...
try:
    includeNames = include_zone_names
except NameError:
    includeNames = False
...
mainLabels["zones"] = Label(
    mainLabels["zframe"], text="Applicable Zones: ", bg=bgmain, fg="#009"
)
...
for id in zone_ids:
    ...
    bt_zones[id] = IntVar()
    mainLabels[id] = Checkbutton(
        mainLabels["zframe"],
        text=id,
        variable=bt_zones[id],
        command=changeZone,
        bg=bgmain,
        highlightthickness=0,
        padx=4,
    ...
    if includeNames:
        ...
        mainLabels[id + "lbl"] = Label(
            mainLabels["zframe"],
            text=labelText,
            bg=bgmain,
            fg="#009",
            font=("Arial", str(int(fontsize) - 3)),
        )
...
if screenRes < 680:
    ...
    wBase = int(0.5 * (screenRes - 680)) + 205

# Put an upper limit on screen height; otherwise, for some monitor configurations,
...
mainLabels["bt_eas"] = Button(
    root, text=" Send to Radio ", command=sendtoRadioCheck, bg="#f33", fg="#ff7"
)
mainLabels["bt_eas"].grid(row=rows, column=0, sticky=W)
mainLabels["expiresin"] = Label(
    mainLabels["bframe"], text="Product expires in: ", bg=bgmain, fg="#00a"
)
mainLabels["expiresin"].grid(row=rows, column=1, sticky=E)
arr = ["10mins", "15mins", "20mins", "30mins", "45mins", "60mins", "90mins", "120mins"]
if "crsexpire" in optset:
    setexpire = optset["crsexpire"].get()
else:
...
mainLabels["bt_close"] = Button(
    root, text=" Close Program ", command=root.destroy, bg="#ccc", fg="#000"
)
mainLabels["bt_close"].grid(
    row=rows + 1, column=0, columnspan=absmaxcol, padx=gp, pady=gp
)


ToolTip(
    mainLabels["bt_eas"],
    text="This is the next step to send the text above to the radio. There will be one more \
...
ToolTip(
    mainLabels["modeme"],
    text="Click Here to toggle between live and practice mode. Live Mode will allow you \
...
ToolTip(
    mainLabels["expiresin"],
    "This allows you to set how long the statement will play on the radio.",
)
...
afterid = root.after(
    250, autoFill
)  # need to pause in case there is a popup window loaded before root. START PROGRAM
root.mainloop()