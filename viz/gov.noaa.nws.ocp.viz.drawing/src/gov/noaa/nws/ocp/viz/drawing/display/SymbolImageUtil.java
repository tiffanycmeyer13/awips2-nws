...
/**
 * This class contains static utility methods for creating symbol/marker images
 * and icons.
 *
 * <pre>
 * SOFTWARE HISTORY
...
public class SymbolImageUtil {
    ...
    private static final IUFStatusHandler handler = UFStatus
            .getHandler(SymbolImageUtil.class);

    // Private constructor
    private SymbolImageUtil() {
    }
    ...
    public static BufferedImage createBufferedImage(String symbolId) {
        return createBufferedImage(symbolId, 2.0, 1.0f, false,
                new Color(0, 0, 0));
    }
     ...
     *
     * @param symbolId
     *            unique identifer for the symbol
     * @param scale
     *            scale factor to use when creating the symbol
     * @param strokeWidth
     *            width of the lines used to create the symbol
     * @param backgroundMask
     *            boolean indicating whether to create a background mask
     * @param symColor
     *            color of symbol
     * @return
     */
    public static BufferedImage createBufferedImage(String symbolId,
            double scale, float strokeWidth, boolean backgroundMask,
            Color symColor) {
        ...
        try {
            ...
            handler.warn("SymbolImageUtil - no pattern found: " + spe);
            return null;
        }
        ...
        if (backgroundMask) {
            ...
            g2d.setStroke(new BasicStroke(strokeWidth + 5.0f,
                    BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            Path2D.Double mask = new Path2D.Double();
            for (ISymbolPart spart : pattern.getParts()) {
                Coordinate[] coords = spart.getPath();
                mask.reset();
                mask.moveTo(center + sfactor * coords[0].x,
                        center - sfactor * coords[0].y);
                for (int i = 1; i < coords.length; i++) {
                    mask.lineTo(center + sfactor * coords[i].x,
                            center - sfactor * coords[i].y);
                }
                g2d.draw(mask);
            }
        }

        // Set color and stroke width for g2d.draw, g2d.fill, g2d.stroke.
        g2d.setColor(symColor);
        g2d.setPaint(symColor);
        ...
        for (ISymbolPart spart : pattern.getParts()) {
            Coordinate[] coords = spart.getPath();
            path.reset();
            path.moveTo(center + sfactor * coords[0].x,
                    center - sfactor * coords[0].y);
            for (int i = 1; i < coords.length; i++) {
                path.lineTo(center + sfactor * coords[i].x,
                        center - sfactor * coords[i].y);
            }
            if (spart.isFilled()) {
                g2d.fill(path);
    ...
    /*
     * The following method was "stolen" from
     * com.raytheon.viz.ui.actions.PrintScreenAction, since it was not declared
     * public.
     */
    public static ImageData convertToSWT(BufferedImage bufferedImage) {
        ...
        if (bufferedImage.getColorModel() instanceof ComponentColorModel) {
            data = convertToSWTComponent(bufferedImage);
        } else if (bufferedImage.getColorModel() instanceof IndexColorModel) {
            data = convertToSWTIndex(bufferedImage);
        }
    ...
    /*
     * Convert based on ComponentColorModel
     *
     * @param bufferedImage
     *
     * @return
     */
    private static ImageData convertToSWTComponent(
        ...
        if (bufferedImage.getColorModel() instanceof ComponentColorModel) {
            ...
            for (int y = 0; y < data.height; y++) {
                for (int x = 0; x < data.width; x++) {
                    ...
                    if (colorModel
                            .getTransparency() == Transparency.TRANSLUCENT) {
    ...
    /*
     * Convert based on IndexColorModel
     *
     * @param bufferedImage
     *
     * @return
     */
    private static ImageData convertToSWTIndex(BufferedImage bufferedImage) {
        ImageData data = null;
