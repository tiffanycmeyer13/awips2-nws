...
/**
 * Utility for tropical cyclone advisory.
 *
 * <pre>
 * SOFTWARE HISTORY
...
public class AdvisoryUtils {
    ...
    private AdvisoryUtils() {
    }

    public static Map<TropicalCycloneAdvisory, String> createSegmentMap(
            TropicalCycloneAdvisory previous, TropicalCycloneAdvisory next) {

        HashMap<TropicalCycloneAdvisory, String> advmap = new HashMap<>();

        if (!(previous.getSegment() instanceof BreakpointPair)
                || !(next.getSegment() instanceof BreakpointPair)) {
            return advmap;
        }
        ...
        if (!coastName.equals(bm.findCoastName(currBkpt1))) {
            return advmap;
        }
        ...
        LineString geom0 = gf.createLineString(new Coordinate[] {
                new Coordinate(pindex1, 0), new Coordinate(pindex2, 0) });
        LineString geom1 = gf.createLineString(new Coordinate[] {
                new Coordinate(cindex1, 0), new Coordinate(cindex2, 0) });

        /*
         * Find the segment common to both. This segment is a "Continuation"
         */
        Geometry result = OverlayOp.overlayOp(geom0, geom1,
                OverlayOp.INTERSECTION);
        if (!result.isEmpty()) {
            int index1 = (int) Math.rint(result.getCoordinates()[0].x);
            int index2 = (int) Math.rint(result.getCoordinates()[1].x);
            BreakpointPair newPair = bm.getBreakpointPair(coastName, index1,
                    index2);
        ...
        /*
         * Find the segments in previous, but not in next. These segments are
         * "Cancel"
         */
        ...
        /*
         * Find the segments in next, but not in previous. These segments are
         * "NEW"
         */
    ...
    public static List<TropicalCycloneAdvisory> segmentAdvisory(
        ...
        List<TropicalCycloneAdvisory> alist = new ArrayList<>();

        BreakpointManager bm = BreakpointManager.getInstance();
        String coast = bm
                .findCoastName(tcadv.getSegment().getBreakpoints().get(0));
        ...
        if (indexes.size() > 2) {
            ...
            for (int j = 0; j < indexes.size() - 1; j++) {
                BreakpointPair newPair = bm.getBreakpointPair(coast, idxs[j],
                        idxs[j + 1]);
